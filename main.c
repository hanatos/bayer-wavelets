...
int main(int argc, char *argv[])
{
  ...
  raw->type = s_buf_raw_stabilise; // instruct that this should be transformed
  ...
  for(int channel=0;channel<3;channel++)
    decompose(raw, coarse0, detail0, channel, 0, sigma_noise);

  for(int channel=0;channel<3;channel++)
    decompose(coarse0, coarse1, detail1, channel, 1, sigma_noise);

  for(int channel=0;channel<3;channel++)
    decompose(coarse1, coarse2, detail2, channel, 2, sigma_noise);
